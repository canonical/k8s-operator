...
def test_invalid_proxy_url(url, detail):
    """Test that the proxy service config handles invalid URLs."""
    with pytest.raises(
        config.proxy_service.ProxyUrlError, match=f"Invalid proxy URL: {url=}"
    ) as ie:
        config.proxy_service._validate_proxy_url(url)
    ie.match(detail)
