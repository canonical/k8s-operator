...
def cloud_profile(
    juju: jubilant.Juju,
    provider: str,
    cloud_marker: Optional[pytest.Mark],  # noqa: ARG001 -- kept for signature parity
) -> None:
    """Apply cloud-specific settings to the model before deploying into it.

    On LXD this applies the machine profile named after the model, so that machines picked
    up by juju inherit it. On EC2/OpenStack it adjusts container networking.

    Args:
        juju: Jubilant Juju instance.
        provider: Cloud provider type, from :func:`cloud_type`.
        cloud_marker: The test module's ``clouds`` marker, if any (unused).
    """
    if provider == "lxd":
        lxd = LXDSubstrate()
