...
`--model`: Run against an existing model instead of creating one -- used together with `--no-deploy` to test a cluster that was deployed some other way.
`--juju-dump-logs`: Write `juju debug-log` for each model before tearing it down.
